...
public class TextureMesh extends TextureAtlasSprite {
	...
	public TextureMesh(String p_i1282_1_, String base, String overlay, Color color, boolean inv) {
		super(p_i1282_1_);
		textureBase = base;
    ...
    public boolean load(IResourceManager manager, ResourceLocation location) {
        ...
        try {
			...
			//e.printStackTrace();
			ElevatorMain.logger.error("Using missing texture, unable to load " + resource2, e);
			int[][] aint = new int[1][];
	        for (int k = 0; k < 1; ++k)
	        {
    ...
    {
        try {
			return (ResourceLocation)ElevatorMain.proxy.resourceLocation.invoke(map, loc, c);
